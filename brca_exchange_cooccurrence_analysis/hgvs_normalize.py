...
def parse_args():
    ...
    parser.add_argument('-i', '--inVCF', type=argparse.FileType('r'),
...
def main(args):
    ...
    vcf_reader = vcf.Reader(options.inVCF)
    vcf_writer = vcf.Writer(open(options.outVCF, 'w'), vcf_reader)
    for record in vcf_reader:
        ...
        hgvs_name = pyhgvs.format_hgvs_name(chrom, offset, ref, alt, genome, transcript, use_gene=False)
        ...
        if len(ref) == len(alt):
            ...
            chrom, offset, ref, alt = pyhgvs.parse_hgvs_name(str(norm_hgvs_c), genome, get_transcript=get_transcript)
        else:
            ...
            print 'vcf_values: {}'.format(vcf_values)
