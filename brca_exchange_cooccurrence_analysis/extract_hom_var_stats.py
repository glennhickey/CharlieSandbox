    ...
    for sample in hom_var_dict.keys():
        if hom_vus_list[sample] > 0:
            if min([float(x) for x in hom_vus_af_list[sample]]) <= 0.01:
                rare_x.append(hom_var_dict[sample])
                rare_y.append(het_dict[sample])
                rare_c.append('lightgreen')
                rare_s.append(20)
                rare_e.append('grey')
            else:
                vus_x.append(hom_var_dict[sample])
                vus_y.append(het_dict[sample])
                vus_c.append('blue')
                vus_s.append(20)
                vus_e.append('blue')
        else:
            nvus_x.append(hom_var_dict[sample])
            nvus_y.append(het_dict[sample])
            nvus_c.append('r')
            nvus_s.append(20)
            nvus_e.append('r')
    fig5, ax5 = matplotlib.pyplot.subplots()
    ax5.scatter(rare_x, rare_y, s=rare_s, color=rare_c, edgecolors=rare_e, zorder=2)
    ax5.scatter(vus_x, vus_y, s=vus_s, color=vus_c, edgecolors=vus_e, zorder=1, alpha=0.3)
    ax5.scatter(nvus_x, nvus_y, s=nvus_s, color=nvus_c, edgecolors=nvus_e, zorder=1, alpha=0.3)
