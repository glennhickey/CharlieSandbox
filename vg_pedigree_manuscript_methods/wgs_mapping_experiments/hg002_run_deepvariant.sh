...
rm merge_${LOW_SAMPLE_NAME}_deepvariant_vcfs.sh
echo -e "#!/bin/bash
cd /data/Udpwork/usr/markellocj/non_segdup_vg_pedigree_runs/${LOW_COHORT_NAME}_cohort/bwamem/bwamem_grch38_${LOW_SAMPLE_NAME}_bam_by_chr ; \
...
&& bgzip -f merged.deepvariant.indel_realigned.sorted.vcf \
&& tabix -p vcf merged.deepvariant.indel_realigned.sorted.vcf.gz" >> merge_${LOW_SAMPLE_NAME}_deepvariant_vcfs.sh
