...
for SIMULATED_READ_DIR in "${SIMULATED_READ_DIR_LIST[@]}" ; do
    ...
    PARAM_PRESET="default" ; \
    PAIRED="-i" ; \
    time vg giraffe -x hg002_cohort/HG002.parental.graphs.xg -H hg002_cohort/HG002.parental.graphs.gbwt -m hg002_cohort/HG002.parental.graphs.min -d hg002_cohort/HG002.parental.graphs.dist -G ${SIMULATED_READ_DIR}/sim.gam -b ${PARAM_PRESET} ${PAIRED} -t 22 > mapped.giraffe_parental.${SIMULATED_READ_DIR}.gam ; \
    time vg gamcompare -r 100 -s <(vg annotate -m -x hg002_cohort/HG002.parental.graphs.xg -a mapped.giraffe_parental.${SIMULATED_READ_DIR}.gam) ${SIMULATED_READ_DIR}/sim.gam 2>count | vg view -aj - > giraffe_parental.compared.json ; \
    GRAPH="hg002_sample_grch38" ; \
    GBWT="sampled.64" ; \
    READS="HG002" ; \
    PARAM_PRESET="default" ; \
