...
module load bbtools seqtk singularity
...
singularity exec -H ${PWD}:${HOME} --pwd ${HOME} \
...
echo simulate reads ; \
time vg sim -r -I -n $NREADS -a -s 12345 -p 570 -v 165 -i 0.00029 -x vg-construct-v3.2.2_grch38-hg005-outstore/hg005_sample_grch38.xg -g vg-construct-v3.2.2_grch38-hg005-outstore/hg005_sample_grch38.gbwt --sample-name HG005 --ploidy-regex "hs38d1:0,chrNC_007605:0,chrX:1,chrY:1,chrY_.*:1,chrEBV:0,.*:2" -F $FASTQ > sim.raw.gam && \
echo annotate with paths \
time vg annotate -p -x vg-construct-v3.2.2_grch38-hg005-outstore/hg005_sample_grch38.vg -a sim.raw.gam > sim.gam && \
